...
#' summary(df_bc[[1]])
#' head(df_bc[[1]])
parse_bioclimatic <- function(path){

  # Parse future directory
  parse_future_dir <- function(path_future){
    options(stringsAsFactors=F)
    fl <- list.files(path_future, full.names = T, recursive = T)
  ...
  parse_historical_dir <- function(path_historical){
    options(stringsAsFactors=F)
    fl <- list.files(path_historical, full.names = T, recursive = T)
    df <- as.data.frame(t(as.data.frame(sapply(fl, strsplit, split="/"))))
    row.names(df) <- NULL
    bn <- unlist(strsplit(df[,ncol(df)], split=".tif"))
    row.names(bn) <- NULL
  ...
  out <- lapply(dl, function(x){
    return(
      merge(
        parse_historical_dir(path_historical=file.path(path, x, "biovar_historical")),
        parse_future_dir(path_future=file.path(path, x, "biovar_future")),
        all=T
        )
    )
  })
